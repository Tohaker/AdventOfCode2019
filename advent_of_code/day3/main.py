    ...
    for command in wire:
        ...
        if direction == 'U':
            for i in range(distance):
                wire_points.append(
                    {'coord': (x, y + i), 'completion': i})
        elif direction == 'R':
            for i in range(distance):
                wire_points.append(
                    {'coord': (x + i, y), 'completion': i})
        elif direction == 'L':
            for i in range(distance):
                wire_points.append(
                    {'coord': (x - i, y), 'completion': i})
...
def wire_length_to_intercept(wire1, wire2, intercept):
    ...
    w1_index = next(i for i, item in enumerate(
        wire1) if item['coord'] == intercept)
    w2_index = next(i for i, item in enumerate(
        wire2) if item['coord'] == intercept)

    # The result is the combined length of each array of steps
    # where progress has been made.
    wire1_length = len(
        list(filter(lambda d: d['completion'] != 0, wire1[:w1_index + 1])))
    wire2_length = len(
        list(filter(lambda d: d['completion'] != 0, wire2[:w2_index + 1])))
...
def part_one(input):
    ...
    intercepts = set([w['coord'] for w in wire1]) & set(
        [w['coord'] for w in wire2])

    return min(filter(bool, map(lambda intercept: abs(
        intercept[0]) + abs(intercept[1]), intercepts)))
